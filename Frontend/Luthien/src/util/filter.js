...
export function filter(raw, filters) {
  ...
  if (checkFilters(filters)) {
    ...
    raw.forEach((hotel) => {
      ...
      if (filters.swimmingPool && hotel.amenities.includes("swimming pool"))
        filtered.push(hotel);
      if (filters.teaMaker && hotel.amenities.includes("tea maker"))
        filtered.push(hotel);
      if (filters.prayerRoom && hotel.amenities.includes("prayer room"))
        filtered.push(hotel);
      if (filters.askInsideRoom && hotel.amenities.includes("ask inside room"))
        filtered.push(hotel);
      if (filters.freeWifi && hotel.amenities.includes("free wifi"))
        filtered.push(hotel);
      if (filters.gym && hotel.amenities.includes("gym")) filtered.push(hotel);
      if (filters.pet && hotel.amenities.includes("pet")) filtered.push(hotel);
      ...
      if (filters.helpBox && hotel.amenities.includes("help box"))
        filtered.push(hotel);
      if (filters.partyServices && hotel.amenities.includes("party services"))
        filtered.push(hotel);
      if (filters.taxi && hotel.amenities.includes("taxi"))
        filtered.push(hotel);
      if (filters.ATM && hotel.amenities.includes("ATM")) filtered.push(hotel);
